...
if DEBUG_FLAG == 1
    ...
    PsychDebugWindowConfiguration([],.8); % display on monitor at half opacity in addition to in the HMD
end
...
%                                                                        shape.disk.X_m, shape.disk.Y_m); % initial plaid poisitions.
        ...
        Screen('Flip', ds.w,ds.vbl + (1 - 0.5) * ds.ifi,[],0);
        %Screen('Flip', ds.w, [],[],1);
        ds.vbl = GetSecs();
     ...
     if movieframe_n == 1  % save first frame of each movie (not the rest for speed)
        ...
        imwrite(M, fullfile(pwd, 'Movie',strcat('neg10_neg5_5_10_holesWithPlaids','.png')));

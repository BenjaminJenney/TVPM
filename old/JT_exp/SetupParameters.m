...
pa.baseDir = pwd;
pa.Desktop =  extractBefore(pwd,'\headRotation');
...
pa.pursuitTargetDist = 20; % depth of target in m

% fixation dot parameters - must be smaller than response target
pa.fixationDiameter = 20;
pa.fixationDist = -19.5; % depth of fixation dot in m
...
% trial timing setup
pa.targetMotionDuration = 1.3; % s
pa.trialNumber = 0; % start at 0
pa.itiLength = .75; % seconds
if strcmp(ds.experimentType,'real')
    pa.recenterScreenLength = 0.7; %0.25 seconds for simulated condition, 0.7 for real head rotation condition %recentering screen duration in seconds
elseif strcmp(ds.experimentType,'simulated') || strcmp(ds.experimentType, 'stabilized')
    pa.recenterScreenLength = 0.2; %0.2 seconds for simulated condition, 0.7 for real head rotation condition %recentering screen duration in seconds
end

% number of trials
pa.nTrials = 75; % Must be divisible by 3 bc there are 3 rot velocities
...
if pa.TRAINING ==1
    ...
    pa.rotationVelocities = [-5 0 5]; % deg/s
end
...
pa.headingAngles = [-10 0 10]; % deg. need to add a small number so staircase thresh never actually converges to exactly this heading and throws error or renders wrong feedback.
...
pa.stepSize = 3.5; %amount staircase is being shifted by initially
pa.minStairThreshold = -28; % deg   Maximum post position must be less than rotation rate * trial length (sec) + monocular horizontal field of view (<35 deg conservatively). Also depends on viewing distance inside hmd. see http://doc-ok.org/?p=1414
pa.maxStairThreshold = 28; % deg
